<template>
  ...
  <CpaProjetosStaticPage v-if="useStaticLayout" :showHeaderFooter="false" />
...
// Usa layout estático quando flag está ativa
definePageMeta({
  ssr: true,
  layout: STATIC_OFFICE365_PRODUCT_ENABLED ? 'static-blank' : 'default'
})
...
// Client-side: detect intl immediately (not waiting for onMounted) so ClientOnly renders correctly
const _clientIsIntl = ref<boolean | null>((() => {
...
const productsIndexPath = computed(() => (effectiveLang.value === 'en' ? '/products' : '/produtos'))
const categoryPathPrefix = computed(() => (effectiveLang.value === 'en' ? '/category' : '/categoria'))
const digitalDeliveryPath = computed(() => (effectiveLang.value === 'en' ? '/digital-delivery' : '/entrega-digital'))
...
const includedItems = computed(() => {
  ...
  const dictFr: Record<string, string> = {
    'Envio imediato após confirmação': 'Livraison rapide apr├¿s confirmation',
    'Envio rápido após confirmação': 'Livraison rapide apr├¿s confirmation',
    ...
    'Acesso digital com conta ativa': 'Acc├¿s numérique avec compte actif',
