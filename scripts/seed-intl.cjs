/**
 * seed-intl.cjs — Precos internacionais (storeSlug='international')
 * Apenas INSERT/UPDATE em ProdutoPrecoMoeda
 * Nao altera: Produto, ProdutoPrecoLoja, precos BRL
 *
 * DRY_RUN=true  node scripts/seed-intl.cjs   -> simulacao
 * DRY_RUN=false node scripts/seed-intl.cjs   -> executa de verdade
 */
'use strict'
...
async function main() {
  ...
  for (const prod of PRICES) {
    for (const [currency, amount] of [['usd', prod.usd], ['eur', prod.eur]]) {
      ...
      if (existing) {
        ...
        console.log('  produtoId  :', prod.produtoId)
        ...
        console.log('  amount     :', existing.amount, '->', amount)
        if (!DRY_RUN) {
  ...
  if (DRY_RUN) {
    ...
    console.log('  DRY_RUN=false node scripts/seed-intl.cjs')
  } else {
