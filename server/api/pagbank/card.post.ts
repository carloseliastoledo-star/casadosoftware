...
import { createPagbankCard } from '../../services/pagbank.js'
...
export default defineEventHandler(async (event) => {
  ...
  const cardExpMonth = body?.card_exp_month
  const cardExpYear = body?.card_exp_year
  ...
  if (!cardExpMonth) throw createError({ statusCode: 400, statusMessage: 'Mês de validade obrigatório' })
  if (!cardExpYear) throw createError({ statusCode: 400, statusMessage: 'Ano de validade obrigatório' })
  ...
  const { order, coupon } = await (prisma as any).$transaction(async (tx: any) => {
    ...
    if (!normalizedCoupon) {
      ...
      if (existing) {
        return { order: existing, coupon: null }
      }
    }
  ...
  let result: any
  try {
    result = await createPagbankCard({
      orderId: order.id,
    ...
    const msg = String(err?.message || 'Erro ao processar cartão no PagBank')
  ...
  if (paid) {
    try {
      ...
      console.error('[pagbank card] fulfillment error:', err)
    }
  }
