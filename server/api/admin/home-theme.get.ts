...
import { defaultHomeTheme, parseHomeThemeJson } from '#root/server/utils/homeTheme'
...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const theme = parseHomeThemeJson(row?.homeThemeJson) ?? defaultHomeTheme()
    return { ok: true, theme }
  } catch (err: any) {
    console.error('[admin/home-theme.get] error:', err?.message || err)
    return { ok: true, theme: defaultHomeTheme() }
  }
})
