...
export default defineEventHandler(async (event) => {
  ...
  for (const tutorial of tutorials) {
    ...
    if (existing) {
      ...
      results.push({ slug: tutorial.slug, action: 'updated', id: existing.id })
    } else {
      ...
      results.push({ slug: tutorial.slug, action: 'created', id: created.id })
    }
  }
  ...
  return {
    success: true,
    results,
    message: `${results.length} tutoriais processados`
  }
})
