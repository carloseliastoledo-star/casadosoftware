  ...
  if (decodedHtmlRaw) {
    try {
      ...
      if (!html && decodedHtmlRaw) {
        console.warn('[blog/post] sanitizeRichHtml returned empty, using raw HTML')
        html = decodedHtmlRaw
      }
    } catch (err) {
      console.error('[blog/post] sanitizeRichHtml failed, using raw HTML:', err)
      html = decodedHtmlRaw
    }
  }
  console.log(`[blog/post] html input len=${String(htmlRaw || '').length}, sanitized len=${String(html || '').length}`)
