...
export default defineEventHandler(async (event) => {
  try {
    ...
    console.error('[ABANDONED_CAPTURE] error:', error)
    // Não falhar a requisição do usuário se falhar ao salvar lead
    return { ok: true, error: 'save_failed', message: String(error?.message || 'unknown error') }
  }
})
